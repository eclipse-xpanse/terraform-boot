...
public class TerraformInstaller {
    ...
    private String installTerraformByRequiredVersion(String requiredOperator,
        ...
        String bestVersionNumber = getBestAvailableVersionMatchingRequiredVersion(
        ...
        String errorMsg = String.format("Installing terraform with version %s into the dir %s "
                + "failed. ", bestVersionNumber, this.terraformInstallDir);
        log.error(errorMsg);
     ...
     */
    private String getBestAvailableVersionMatchingRequiredVersion(String requiredOperator,
                                                                  String requiredNumber) {
        Set<String> availableVersions = this.versionsCache.getAvailableVersions();
        String bestAvailableVersion =
                this.versionHelper.findBestVersionFromAllAvailableVersions(availableVersions,
                        requiredOperator, requiredNumber);
        if (StringUtils.isNotBlank(bestAvailableVersion)) {
            log.info("Found the best available version {} for terraform by the required version "
                    + "{}.", bestAvailableVersion, requiredOperator + requiredNumber);
            return bestAvailableVersion;
        }
        String errorMsg = String.format("Failed to find available versions for terraform by the "
                + "required version %s.", requiredOperator + requiredNumber);
        log.error(errorMsg);
        throw new InvalidTerraformToolException(errorMsg);
    }
